...
// "Near You": city + mile-radius filter over independent brewery partners,
// in the toolbar alongside Sort/Type/Stores. PARTNER_BREWERIES has no
// members yet (and none have a location on file), so there's nothing to
// filter by distance against, showIndependent() below always falls through
// to the same waiting message regardless of what's typed here. Once real
// partners are signed up with a town/location, this is where the distance
// filtering plugs in: geocode nearbyCity against a static UK town lookup,
// then keep only partner breweries within nearbyRadiusMiles of it.
let nearbyRadiusMiles = 5;

function setNearbyRadius(miles) {
    nearbyRadiusMiles = Number(miles);
}
